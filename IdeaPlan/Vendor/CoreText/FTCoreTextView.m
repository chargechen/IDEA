...
//                    NSURL *url = [NSURL URLWithString:elementContent];
...
//            UIImage *img =  [UIImage imageWithData:[NSData dataWithContentsOfURL:url]];
			if (1) {
//				int x = 0;
//				if (alignment == kCTRightTextAlignment) x = (self.frame.size.width - screenWidth());
//				if (alignment == kCTCenterTextAlignment) x = ((screenWidth()- self.frame.size.width) / 2);
				
				CGRect frame = CGRectMake(0, (lineFrame.origin.y - 100), self.frame.size.width, 100);
                ...
                {
                    frame.origin.x = (alignment == kCTLeftTextAlignment)? insets.left : (self.frame.size.width - screenWidth() - insets.right);
                }
                
                frame.origin.y += insets.top;
//                frame.size.width = ((insets.left + insets.right + screenWidth() ) > self.frame.size.width)? self.frame.size.width : screenWidth();
                
                LZWidgetPhotoView *photo = [[LZWidgetPhotoView alloc] initWithFrame:frame];
                [photo setFileID:imageNode.imageName];

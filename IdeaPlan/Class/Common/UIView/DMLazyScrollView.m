...
- (id)initWithFrame:(CGRect)frame
{
    return [self initWithFrame:frame
                     direction:DMLazyScrollViewDirectionHorizontal
                circularScroll:YES];
}
...
- (void)resetAutoPlay
{
    if(_autoPlay)
    {
        ...
        _timer_autoPlay = [NSTimer scheduledTimerWithTimeInterval:_autoPlayTime
                                                           target:self
                                                         selector:@selector(autoPlayHanlde:)
                                                         userInfo:nil
                                                          repeats:YES];
    }
    else
...
-(void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate{
    self.bounces = YES;
    if (nil != _controlDelegate && [_controlDelegate respondsToSelector:@selector(lazyScrollViewDidEndDragging:)])
        [_controlDelegate lazyScrollViewDidEndDragging:self];
    [self autoPlayResume];
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    [self autoPlayPause];
    if (nil != _controlDelegate && [_controlDelegate respondsToSelector:@selector(lazyScrollViewWillBeginDragging:)])
        [_controlDelegate lazyScrollViewWillBeginDragging:self];
}

- (void) scrollViewDidScroll:(UIScrollView *)scrollView {
    if (isManualAnimating) {
        if (nil != _controlDelegate && [_controlDelegate respondsToSelector:@selector(lazyScrollViewDidScroll:at:withSelfDrivenAnimation:)]) {
            [_controlDelegate lazyScrollViewDidScroll:self at:[self visibleRect].origin withSelfDrivenAnimation:YES];
        }
        return;
    }
    ...
    // alert delegate
    if (nil != _controlDelegate && [_controlDelegate respondsToSelector:@selector(lazyScrollViewDidScroll:at:withSelfDrivenAnimation:)]) {
        [_controlDelegate lazyScrollViewDidScroll:self at:[self visibleRect].origin withSelfDrivenAnimation:NO];
    }
    else if (nil != _controlDelegate && [_controlDelegate respondsToSelector:@selector(lazyScrollViewDidScroll:at:)]) {
        [_controlDelegate lazyScrollViewDidScroll:self at:[self visibleRect].origin];
    }
}
...
- (void)setPage:(NSInteger)newIndex
     transition:(DMLazyScrollViewTransition)transition
...
- (UIViewController *)loadControllerAtIndex:(NSInteger)index
                            andPlaceAtIndex:(NSInteger)destIndex {
    
    UIViewController *viewController = _dataSource(index);
...
- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView {
    if (nil != _controlDelegate && [_controlDelegate respondsToSelector:@selector(lazyScrollViewWillBeginDecelerating:)])
        [_controlDelegate lazyScrollViewWillBeginDecelerating:self];
}

- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (nil != _controlDelegate && [_controlDelegate respondsToSelector:@selector(lazyScrollViewDidEndDecelerating:atPageIndex:)])
        [_controlDelegate lazyScrollViewDidEndDecelerating:self atPageIndex:self.currentPage];
}
